...
export async function downloadSounds(list, apiKey) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    let author = cleanFileName(responseData.data.author);
    ...
    try {
      ...
      file.on('finish', () => {
        console.log(
          chalk.greenBright(`Finished downloading sound ` + soundURL)
        );
        file.close();
      });
      ...
      console.error(chalk.red('Error saving file to disk\n' + error));
    }
